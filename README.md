...
This project implements a status page that displays the current and historical uptime for various monitored services. The frontend is designed to be hosted on GitHub Pages (or another static hosting provider), and the backend is a Node.js proxy server designed to be run with Docker and an Nginx reverse proxy.
...
│   └── config.js           # Frontend configuration (service definitions, API endpoint)
├── backend/
│   ├── proxy-server.js     # Node.js/Express backend proxy
...
*   **Configurable Services:** Services are defined in `frontend/config.js` (for display) and `backend/proxy-services-config.yaml` (for Prometheus query details).
...
1.  **Configure API Endpoint:**
    ...
    *   Set the `PROMETHEUS_URL` to your backend API endpoint:
        ```javascript
...
2.  **Deploy to GitHub Pages:**
    ...
    *   Push your project code (including the `frontend` directory) to the repository.
    *   In your GitHub repository settings, go to the "Pages" section.
    *   Choose the branch you want to deploy from (e.g., `main`).
    *   Select the `/frontend` folder as the source for GitHub Pages.
    *   Save the changes.
...
*   **`frontend/index.html`**: The main page structure.
*   **`frontend/style.css`**: All visual styling.
*   **`frontend/script.js`**: Core frontend logic, data fetching, UI updates.
*   **`frontend/config.js`**: Defines services shown on the page and the backend API URL.
*   **`backend/proxy-server.js`**: Node.js application that proxies requests to Prometheus, applies whitelisting/security logic.
...
*   **`backend/.env`**: Stores environment-specific configuration for the backend (e.g., Prometheus URL, port). **Crucially, this file is NOT committed to Git.**
